 ...
 */
bool BattleActionCost::haveTU(std::string *message, bool allowDebt)
{
	if (Time <= 0)
	{
	...
	if (!actor->canSpendTimeUnits(Time, allowDebt))
	{
		if (message)
		{
 ...
 */
bool BattleActionCost::spendTU(std::string *message, bool allowDebt)
{
	if (haveTU(message))
	{
...
void BattlescapeGame::handleAI(BattleUnit *unit)
{
	...
	if (action.type == BA_SNAPSHOT || action.type == BA_AUTOSHOT || action.type == BA_AIMEDSHOT || action.type == BA_BURSTSHOT || action.type == BA_THROW || action.type == BA_HIT || action.type == BA_MINDCONTROL || action.type == BA_USE || action.type == BA_PANIC || action.type == BA_LAUNCH || action.type == BA_CLAIRVOYANCE || action.type == BA_MINDBLAST || action.type == BA_DUALFIRE)
	{
		ss.clear();
		ss << L"Attack type=" << action.type << " target="<< action.target << " weapon=" << Language::utf8ToWstr(action.weapon->getRules()->getName());
		_parentState->debug(ss.str());
		if (action.type == BA_MINDCONTROL || action.type == BA_PANIC || action.type == BA_USE || action.type == BA_CLAIRVOYANCE || action.type == BA_MINDBLAST)
		{
...
bool BattlescapeGame::kneel(BattleUnit *bu)
{
	...
	if (bu->getType() == "SOLDIER" && bu->getArmor()->allowsKneeling()  && !bu->isVehicle() && !bu->isFloating() && ((!bu->isKneeled() && _save->getKneelReserved()) || checkReservedTU(bu, tu, 0)))
	{
		BattleAction kneel;
		kneel.type = BA_KNEEL;
		kneel.actor = bu;
 ...
 */
void BattlescapeGame::checkForCasualties(const RuleDamageType *damageType, BattleActionAttack attack, bool hiddenExplosion, bool terrainExplosion, bool pushDeathFront)
{
	...
	for (std::vector<BattleUnit*>::iterator j = _save->getUnits()->begin(); j != _save->getUnits()->end(); ++j)
	{
		...
		{
			if (victim->getHealth() <= victim->getDeathHealth())
			{
		...
		if (!murderer && !terrainExplosion)
		{
			for (std::vector<BattleUnit*>::const_iterator i = _save->getUnits()->begin(); i != _save->getUnits()->end(); ++i)
			{
				if ((*i)->getId() == victim->getMurdererId())
				{
					...
					murderer->addBattleExperience("STR_KILL");
					break;
				}
			}
		}
		...
		if ((*j)->getStatus() != STATUS_DEAD)
		{
			if ((*j)->getHealth() <= (*j)->getDeathHealth())
			{
				if (murderer)
				{
				...
				victim->backlashController(getTileEngine(), _parentState->getCombatLog());

				if (damageType)
				{
					statePushNext(new UnitDieBState(this, (*j), damageType, noSound, false));
				}
				else
				{
					if (hiddenExplosion)
					{
						...
						noSound = true;
						deathState = new UnitDieBState(this, (*j), getMod()->getDamageType(DT_HE), noSound, true);
					}
					else
					{
						if (terrainExplosion)
						{
							// terrain explosion
							deathState = new UnitDieBState(this, (*j), getMod()->getDamageType(DT_HE), noSound, false);
						}
						else
						{
							// no murderer, and no terrain explosion, must be fatal wounds
							deathState = new UnitDieBState(this, (*j), getMod()->getDamageType(DT_NONE), noSound, false);  // DT_NONE = STR_HAS_DIED_FROM_A_FATAL_WOUND
						}
					}
				}
			...
			else if ((*j)->getStunlevel() >= (*j)->getHealth() && (*j)->getStatus() != STATUS_UNCONSCIOUS)
			{
				...
				if (murderer && (victim->getOriginalFaction() == FACTION_HOSTILE && murderer->getOriginalFaction() == FACTION_PLAYER))
				{
					murderer->addBattleExperience("STR_STUN");
				}
				victim->setJustKilled(true);
				noSound = true;
				deathState = new UnitDieBState(this, (*j), getMod()->getDamageType(DT_STUN), noSound, false);
			}
		}

		if(deathState)
		{
			getTileEngine()->calculateFOV(deathState->getUnit()->getPosition());
			if(pushDeathFront)
			{
...
void BattlescapeGame::handleNonTargetAction()
{
	if (!_currentAction.targeting)
	{
		...
		else if (_currentAction.type == BA_PRIME && _currentAction.value != BattleItem::GRENADE_INACTIVE)
		{
		...
		else if(_currentAction.type == BA_RELOAD)
		{
			...
			else if(_currentAction.ammo)
			{
				//BattleItem *quickAmmo = _currentAction.actor->findQuickAmmo(_currentAction.weapon);
				//
				///*bool drop = true;
				//if(_currentAction.actor->isVehicle())
				//{
				//}*/

				//RuleInventory *ground = getMod()->getInventory("STR_GROUND");
				...
				reloadLog << _currentAction.actor->getLogName(_parentState->getGame()->getLanguage()) << " > " << _parentState->tr("STR_RELOADED").arg(_parentState->tr(_currentAction.weapon->getRules()->getName())).arg(_parentState->tr(_currentAction.ammo->getRules()->getName()));

				_parentState->combatLog(reloadLog.str(), COMBAT_LOG_GREEN);

				getMod()->getSoundByDepth(_save->getDepth(), Mod::ITEM_RELOAD)->play(-1, getMap()->getSoundAngle(_save->getSelectedUnit()->getPosition()));
			}
		}
		else if(selected && _currentAction.type != BA_NONE && _currentAction.type == selected->getOngoingAction())
		{
			if (_currentAction.actor->spendTimeUnits(_currentAction.Time))
			{
				switch(_currentAction.type)
				{
				case BA_OVERWATCH:
					{
						getMod()->getSound("BATTLE.CAT", 17)->play();
						_save->getSelectedUnit()->clearOverwatch();
						_parentState->updateSoldierInfo();
						Game *game = _parentState->getGame();
						std::wostringstream log;
						log << game->getLanguage()->getString("STR_LOG_ACTOR").arg(_save->getSelectedUnit()->getLogName(game->getLanguage())) << game->getLanguage()->getString("STR_LOG_CANCEL_OVERWATCH");
						_parentState->combatLog(log.str(), COMBAT_LOG_GREEN);
					}
					break;
				case BA_MINDCONTROL:
					{
						getMod()->getSound("BATTLE.CAT", 36)->play();
						_save->getSelectedUnit()->clearControlling();
						_parentState->updateSoldierInfo();
						Game *game = _parentState->getGame();
						std::wostringstream log;
						log << game->getLanguage()->getString("STR_LOG_ACTOR").arg(_save->getSelectedUnit()->getLogName(game->getLanguage())) << game->getLanguage()->getString("STR_LOG_CANCEL_MIND_CONTROL");
						_parentState->combatLog(log.str(), COMBAT_LOG_GREEN);
					}
					break;

				default:
					selected->clearOngoingAction();
				}
			}
			else
			{
...
void BattlescapeGame::setupCursor()
{
	if (_currentAction.targeting)
	{
		...
		else if (_currentAction.type == BA_MINDCONTROL || _currentAction.type == BA_PANIC || _currentAction.type == BA_USE || _currentAction.type == BA_CLAIRVOYANCE)
		{
...
void BattlescapeGame::popState()
{
	...
	if (action.movementAction != BAM_NORMAL)
	{
		action.actor->setMovementAction(BAM_NORMAL);
		//_parentState->combatLog(_parentState->tr("STR_LOG_ACTOR_ACTION").arg(_currentAction.actor->getLogName(Game::getLanguage())).arg("-Sprinting"), COMBAT_LOG_GREEN);
	}
	...
	if (action.actor && noActionsPending(action.actor))
	{
		if (action.actor->getFaction() == FACTION_PLAYER)
		{
			if (_save->getSide() == FACTION_PLAYER)
			{
				...
				{
					RuleItemUseCost cost = _currentAction.actor->getActionTUs(_currentAction.actor->getOverwatchShotAction(_currentAction.weapon->getRules()), _currentAction.weapon);

					if(action.actor->spendTimeUnits(cost.Time))
					{
						getMod()->getSound("BATTLE.CAT", 17)->play();
						_save->getSelectedUnit()->activateOverwatch(_save, _currentAction.weapon, _currentAction.target);
						_parentState->updateSoldierInfo();
						//setupCursor();
						Game *game = _parentState->getGame();
						std::wostringstream log;
						log << game->getLanguage()->getString("STR_LOG_ACTOR").arg(_save->getSelectedUnit()->getLogName(game->getLanguage())) << game->getLanguage()->getString("STR_LOG_OVERWATCH");
						_parentState->combatLog(log.str(), COMBAT_LOG_GREEN);
					}
					else
					{
				...
				else if(action.type == BA_MINDCONTROL && action.actor->getOngoingAction() == BA_MINDCONTROL)
				{
					cancelCurrentAction(true);
				}
...
bool BattlescapeGame::checkReservedTU(BattleUnit *bu, int tu, int energy, bool justChecking)
{
	...
	if (_save->getSide() == FACTION_HOSTILE && !_debugPlay) // aliens reserve TUs as a percentage rather than just enough for a single action.
	{
		...
		return cost.Time <= 0 || cost.haveTU(&dummy, false);
	}
	...
	if (!cost.haveTU(&dummy, false) && !justChecking)
	{
		return true;
	}
	...
	if ((cost.type != BA_NONE || _save->getKneelReserved()) && !cost.haveTU(&dummy, false))
	{
		if (!justChecking)
		{
			if (tuKneel)
...
bool BattlescapeGame::handlePanickingUnit(BattleUnit *unit)
{
	...
	{
		BattleItem *item = unit->getWeapon1();
		if (item)
		{
			dropItem(unit->getPosition(), item, false, true);
		}
		item = unit->getWeapon2();
		if (item)
		{
			dropItem(unit->getPosition(), item, false, true);
		}
...
void BattlescapeGame::primaryAction(Position pos)
{
	...
	if (_currentAction.targeting && _save->getSelectedUnit())
	{
		if (_currentAction.type == BA_LAUNCH)
		{
			...
			if (maxWaypoints == 0)
			{
				maxWaypoints = _currentAction.weapon->getAmmoForAction(BA_LAUNCH)->getRules()->getWaypoints();
			}
		...
		else if (_currentAction.type == BA_PANIC || _currentAction.type == BA_MINDCONTROL || _currentAction.type == BA_CLAIRVOYANCE || _currentAction.type == BA_MINDBLAST || (_currentAction.type == BA_USE && _currentAction.weapon->getRules()->getBattleType() == BT_PSIAMP))
		{
			BattleUnit *actor = _save->getSelectedUnit();
			BattleUnit *target;
			if (_currentAction.type == BA_CLAIRVOYANCE || 
				((target = _save->selectUnit(pos))
				 && target->getFaction() != actor->getFaction()
				 && target->getVisible()))
			{
				_currentAction.updateTU();
				_currentAction.target = pos;
				if (_currentAction.type == BA_CLAIRVOYANCE
					|| !_currentAction.weapon->getRules()->isLOSRequired()
					|| std::find(_currentAction.actor->getVisibleUnits()->begin(), _currentAction.actor->getVisibleUnits()->end(), _save->selectUnit(pos)) != _currentAction.actor->getVisibleUnits()->end())
				{
	...
	else
	{
		...
		{
			SDLMod modifierPressed = SDL_GetModState();

			if (bPreviewed &&
				(_currentAction.target != pos || (_save->getPathfinding()->getModifierUsed() != modifierPressed)))
			{
				_save->getPathfinding()->removePreview();
			}
			_currentAction.target = pos;
			_save->getPathfinding()->calculate(_currentAction.actor, _currentAction.target);
			_currentAction.movementAction = _save->getPathfinding()->getMovementAction();
			// if running or shifting, ignore spotted enemies (i.e. don't stop)
			_currentAction.ignoreSpottedEnemies = (_currentAction.movementAction == BAM_RUN) || ((modifierPressed & KMOD_SHIFT) == modifierPressed);
			...
			{
				//  -= start walking =-
				//_currentAction.actor->setSprinting(_currentAction.movementAction == MV_);
				_currentAction.actor->setMovementAction(_currentAction.movementAction);
...
void BattlescapeGame::secondaryAction(Position pos)
{
	...
	_currentAction.target = pos;
	_currentAction.actor = _save->getSelectedUnit();
	_currentAction.movementAction = (Options::strafe && (SDL_GetModState() & KMOD_CTRL) != 0 && _save->getSelectedUnit()->getTurretType() > -1) ? BAM_STRAFE : BAM_NORMAL;
 ...
 */
void BattlescapeGame::dropItem(Position position, BattleItem *item, bool removeItem, bool updateLight, bool newItem)
{
...
bool BattlescapeGame::takeItem(BattleItem* item, BattleAction *action)
{
	...
	auto rightWeapon = action->actor->getWeapon1();
	auto leftWeapon = action->actor->getWeapon2();
	...
	auto reloadWeapon = [&unit](BattleItem* weapon, BattleItem* i)
	{
		if (weapon && weapon->isWeaponWithAmmo() && !weapon->haveAllAmmo())
		{
			...
			if (slot != -1)
			{
				// TODO: Check that this matches reload costs.
...
bool BattlescapeGame::isSurrendering(BattleUnit* bu)
{
	...
	else if (surrenderMode == 3)
	{
		...
		if (!bu->getWeapon1() && !bu->getWeapon2() && bu->wantsToSurrender())
		{
			return true;
		}
	}
