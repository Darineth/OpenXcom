...
RoleChangeState::RoleChangeState(RoleMenuState* parentState) : _parentState(parentState)
{
	...
	// Set palette
	setInterface("rolechange", false, _game->getSavedGame() ? _game->getSavedGame()->getSavedBattle() : 0);

	// As per PrimeGrenadeState
	setPalette("PAL_BATTLESCAPE");

	add(_window, "window", "rolechange");
	add(_rolesList, "list", "rolechange");
	add(_btnCancel, "button", "rolechange");
	...
	std::vector<std::string> roles = _game->getMod()->getRolesList();
...
{
	_parentState->changeRole(_game->getMod()->getRolesList()[_rolesList->getSelectedRow()]);
	_game->popState();
}
}
