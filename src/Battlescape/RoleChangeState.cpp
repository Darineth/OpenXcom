 ...
 */
RoleChangeState::RoleChangeState(Game *game, RoleMenuState* parentState) : State(game), _parentState(parentState)
{
	_screen = false;
	...
	std::vector<std::string> roles = game->getRuleset()->getRolesList();
