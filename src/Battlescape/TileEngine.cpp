...
int TileEngine::distance(const Position &pos1, const Position &pos2) const
{
	int x = pos1.x - pos2.x;
	int y = pos1.y - pos2.y;
	/*float sqr = float(x*x + y*y);
	float distance = sqrt(sqr);
...
void TileEngine::getTargetVoxel(BattleAction *action, const Position &target, Position &targetVoxel)
{
	...
	{
		if (!canTargetTile(&originVoxel, targetTile, MapData::O_OBJECT, &targetVoxel, actor))
		{
	...
	{
		if (!canTargetTile(&originVoxel, targetTile, MapData::O_NORTHWALL, &targetVoxel, actor))
		{
	...
	{
		if (!canTargetTile(&originVoxel, targetTile, MapData::O_WESTWALL, &targetVoxel, actor))
		{
	...
	{
		if (!canTargetTile(&originVoxel, targetTile, MapData::O_FLOOR, &targetVoxel, actor))
		{
