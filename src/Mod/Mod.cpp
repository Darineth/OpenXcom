...
Mod::Mod() :
	...
	_aiNormalTUReserve(false), _aiCombatTUReserve(false),
...
void Mod::loadFile(const FileMap::FileRecord &filerec, ModScript &parsers)
{
	...
	if (const auto& nodeAI = loadDocInfoHelper("ai"))
	{
		...
		nodeAI.tryRead("normalTUReserve", _aiNormalTUReserve);
		nodeAI.tryRead("combatTUReserve", _aiCombatTUReserve);
	...
	{
		Armor::bleedoutDefaults.load(bleedoutDefaults);
	}
