...
struct UnitStats
{
	int tu, stamina, health, bravery, reactions, firing, throwing, strength, psiStrength, psiSkill, melee, piloting, unguided, guided;
public:
	UnitStats() : tu(0), stamina(0), health(0), bravery(0), reactions(0), firing(0), throwing(0), strength(0), psiStrength(0), psiSkill(0), melee(0), piloting(0), unguided(0), guided(0) {};
	UnitStats(int tu_, int stamina_, int health_, int bravery_, int reactions_, int firing_, int throwing_, int strength_, int psiStrength_, int psiSkill_, int melee_, int piloting_, int unguided_, int guided_) : tu(tu_), stamina(stamina_), health(health_), bravery(bravery_), reactions(reactions_), firing(firing_), throwing(throwing_), strength(strength_), psiStrength(psiStrength_), psiSkill(psiSkill_), melee(melee_), piloting(piloting_), unguided(unguided_), guided(guided_) {};
	UnitStats(const UnitStats &stats) : tu(stats.tu), stamina(stats.stamina), health(stats.health), bravery(stats.bravery), reactions(stats.reactions), firing(stats.firing), throwing(stats.throwing), strength(stats.strength), psiStrength(stats.psiStrength), psiSkill(stats.psiSkill), melee(stats.melee), piloting(stats.piloting), unguided(stats.unguided), guided(stats.guided) {};
	UnitStats& operator=(const UnitStats& stats) { tu = stats.tu; stamina = stats.stamina; health = stats.health; bravery = stats.bravery; reactions = stats.reactions; firing = stats.firing; throwing = stats.throwing; strength = stats.strength; psiStrength = stats.psiStrength; psiSkill = stats.psiSkill; melee = stats.melee; piloting = stats.piloting; unguided = stats.unguided; guided = stats.guided; return *this; }
	UnitStats& operator+=(const UnitStats& stats) { tu += stats.tu; stamina += stats.stamina; health += stats.health; bravery += stats.bravery; reactions += stats.reactions; firing += stats.firing; throwing += stats.throwing; strength += stats.strength; psiStrength += stats.psiStrength; psiSkill += stats.psiSkill; melee += stats.melee; piloting += stats.piloting; unguided += stats.unguided; guided += stats.guided; return *this; }
	UnitStats operator+(const UnitStats& stats) const { return UnitStats(tu + stats.tu, stamina + stats.stamina, health + stats.health, bravery + stats.bravery, reactions + stats.reactions, firing + stats.firing, throwing + stats.throwing, strength + stats.strength, psiStrength + stats.psiStrength, psiSkill + stats.psiSkill, melee + stats.melee, piloting + stats.piloting, unguided + stats.unguided, guided + stats.guided); }
	UnitStats& operator-=(const UnitStats& stats) { tu -= stats.tu; stamina -= stats.stamina; health -= stats.health; bravery -= stats.bravery; reactions -= stats.reactions; firing -= stats.firing; throwing -= stats.throwing; strength -= stats.strength; psiStrength -= stats.psiStrength; psiSkill -= stats.psiSkill; melee -= stats.melee; piloting -= stats.piloting; unguided -= stats.unguided; guided -= stats.guided; return *this; }
	UnitStats operator-(const UnitStats& stats) const { return UnitStats(tu - stats.tu, stamina - stats.stamina, health - stats.health, bravery - stats.bravery, reactions - stats.reactions, firing - stats.firing, throwing - stats.throwing, strength - stats.strength, psiStrength - stats.psiStrength, psiSkill - stats.psiSkill, melee - stats.melee, piloting - stats.piloting, unguided - stats.unguided, guided - stats.guided); }
	UnitStats operator-() const { return UnitStats(-tu, -stamina, -health, -bravery, -reactions, -firing, -throwing, -strength, -psiStrength, -psiSkill, -melee, -piloting, -unguided, -guided); }
	UnitStats operator*(const UnitStats& mods) { return UnitStats(mod(tu, mods.tu), mod(stamina, mods.stamina), mod(health, mods.health), mod(bravery, mods.bravery), mod(reactions, mods.reactions), mod(firing, mods.firing), mod(throwing, mods.throwing), mod(strength, mods.strength), mod(psiStrength, mods.psiStrength), mod(psiSkill, mods.psiSkill), mod(melee, mods.melee), mod(piloting, mods.piloting), mod(unguided, mods.unguided), mod(guided, mods.guided)); }
	UnitStats& operator*=(const UnitStats& mods) { tu = mod(tu, mods.tu); stamina = mod(stamina, mods.stamina); health = mod(health, mods.health); bravery = mod(bravery, mods.bravery); reactions = mod(reactions, mods.reactions); firing = mod(firing, mods.firing); throwing = mod(throwing, mods.throwing); strength = mod(strength, mods.strength); psiStrength = mod(psiStrength, mods.psiStrength); psiSkill = mod(psiSkill, mods.psiSkill); melee = mod(melee, mods.melee); piloting = mod(piloting, mods.piloting); unguided = mod(unguided, mods.unguided); guided = mod(guided, mods.guided); return *this; }
	void merge(const UnitStats& stats) { tu = (stats.tu ? stats.tu : tu); stamina = (stats.stamina ? stats.stamina : stamina); health = (stats.health ? stats.health : health); bravery = (stats.bravery ? stats.bravery : bravery); reactions = (stats.reactions ? stats.reactions : reactions); firing = (stats.firing ? stats.firing : firing); throwing = (stats.throwing ? stats.throwing : throwing); strength = (stats.strength ? stats.strength : strength); psiStrength = (stats.psiStrength ? stats.psiStrength : psiStrength); psiSkill = (stats.psiSkill ? stats.psiSkill : psiSkill); melee = (stats.melee ? stats.melee : melee); piloting = (stats.piloting ? stats.piloting : piloting); unguided = (stats.unguided ? stats.unguided : unguided); guided = (stats.guided ? stats.guided : guided); };
	void clear() { tu = 0; stamina = 0; health = 0; bravery = 0; reactions = 0; firing = 0; throwing = 0; strength = 0; psiStrength = 0; psiSkill = 0; melee = 0; piloting = 0; guided = 0; unguided = 0; }
	bool isZero() { return tu == 0 && stamina == 0 && health == 0 && bravery == 0 && reactions == 0 && firing == 0 && throwing == 0 && strength == 0 && psiStrength == 0 && psiSkill == 0 && melee == 0 && piloting == 0 && guided == 0 & unguided == 0; }
private:
...
namespace YAML
{
	...
	struct convert<OpenXcom::UnitStats>
	{
		static Node encode(const OpenXcom::UnitStats& rhs)
		{
			...
			node["piloting"] = rhs.piloting;
			node["unguided"] = rhs.unguided;
			node["guided"] = rhs.guided;
			return node;
		}
		...
		static bool decode(const Node& node, OpenXcom::UnitStats& rhs)
		{
			...
			rhs.piloting = node["piloting"].as<int>(rhs.piloting);
			rhs.unguided = node["unguided"].as<int>(rhs.unguided);
			rhs.guided = node["guided"].as<int>(rhs.guided);
			return true;
		}
	};
}
