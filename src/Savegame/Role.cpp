...
{
/// Creates a new role.
Role::Role(std::string name, RuleRole *rules) : _name(name), _rules(rules), _defaultLayout(), _defaultArmorColor(rules->getDefaultArmorColor())
{
}
...
{
	_defaultArmorColor = _rules->getDefaultArmorColor();
...
YAML::Node Role::save() const
{
	...
	node["name"] = _name;
	node["defaultArmorColor"] = _defaultArmorColor;
...
void Role::setDefaultLayout(const std::vector<EquipmentLayoutItem*> &layout, const std::string &armorColor)
{
	clearDefaultLayout();

	_defaultArmorColor = armorColor;
...
const std::string &Role::getDefaultArmorColor() const
{
